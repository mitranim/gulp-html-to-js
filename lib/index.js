...
module.exports = options => (
  ...
  ? concatTransform(options)
  : separateTransform()
)

function concatTransform (options) {
  ...
  return new Transform({
    ...
    flush (done) {
      done(undefined, this.push(new File({
        path: options.concat,
        contents: Buffer.from(join(results, options))
      })))
    }
  })
}

function separateTransform () {
  return new Transform({
    objectMode: true,
    transform (file, _, done) {
      if (file.isBuffer()) {
          ...
          contents: Buffer.from(textToModule(file.contents.toString()))
        }))
      }
    },
  })
...
function textToExport (text, path, options) {
  if (options.prefix) {
    path = pt.join(options.prefix, path)
  }
...
// Convert backslashes to slashes in case we got a Windows path
